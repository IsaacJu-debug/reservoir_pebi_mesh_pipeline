function nPrisms = writeVTKFieldData( G, rock, numFacesAlongFault, filename, ...
                                        Geo, isSplited)
  % Geo is an input struct
  ...
  cell_markers = ones(G.cells.num + numFacesAlongFault, 1); % Initialize all as reservoir

  % Define overburden
  % For cells with centroids in positive x direction and above a certain depth
  overburdenCondition1 = (G.cells.centroids(:,1) > 0.0) & (G.cells.centroids(:,2) > Geo.fracDepth(1));
  cell_markers(overburdenCondition1) = 2;

  % For cells with centroids in negative x direction and above a certain depth
  overburdenCondition2 = (G.cells.centroids(:,1) < 0.0) & (G.cells.centroids(:,2) > Geo.fracDepth(2));
  cell_markers(overburdenCondition2) = 2;

  % Define underburden
  % For cells with centroids in positive x direction and below a certain depth
  underburdenCondition1 = (G.cells.centroids(:,1) > 0.0) & (G.cells.centroids(:,2) < Geo.fracDepth(3));
  cell_markers(underburdenCondition1) = 3;

  % For cells with centroids in negative x direction and below a certain depth
  underburdenCondition2 = (G.cells.centroids(:,1) < 0.0) & (G.cells.centroids(:,2) < Geo.fracDepth(4));
  cell_markers(underburdenCondition2) = 3;
  ...
  if isSplited
      ...
      fault_markers = zeros(G.cells.num, 1); % Initialize for the rest of the domain

      for iFace = 1:numFacesToBeSplited
          faceID = faceArray(iFace);

          % Check if the face centroid's x-coordinate is within a small range around 0
          if abs(G.faces.centroids(faceID, 1)) <= epsilon
              % Mark the cells on either side of the face with different markers
