...
file_name = '_incline_frac.vtk'; % file name
...
nList   = [10]; % Approximate number of cells in x-direction
...
for i = 1:size(nList, 2)
    ...
    if containFracture
            ...
            [G2D, Pts ] = pebiGrid2D( ...
                ...
                'FCFactor'      , 0.01           , ... % Relative size of fault cells
                'FCEps'         , 0.05*max(domainSize), ... % the smaller, the more adrupt the change is
                'useMrstPebi', true);
    ...
    file_name = strcat( '_res', num2str(n), '_', file_name);
    vtk_name = strcat( num2str(i, '%0.4d'), file_name );
    disp(strcat('Writing ', vtk_name));
